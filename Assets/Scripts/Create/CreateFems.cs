...
public class CreateFems : MonoBehaviour {
    ...
    void onClickOKSca()
    {
        ...
        Parallel.For(0, parallelNum, new ParallelOptions { MaxDegreeOfParallelism = parallelNum }, (paraIndex) =>
         {
             BaseClass1(timeNum * paraIndex / parallelNum, timeNum * (paraIndex + 1) / parallelNum);
         });
    ...
    void BaseClass1(int startTime, int endTime)
    {
        ...
        for (int time = startTime; time < endTime; ++time)
        {
            ...
            for (int scaIndex = 0; scaIndex < scalarsNum; ++scaIndex)
            {
                float[][] scalars = new float[domainNum][];
                float scaMax = float.MinValue, scaMin = float.MaxValue;
                string sName = scaName[scaNum[scaIndex]];
                string scaName_ = sName + " ";
                for (int domain = 0; domain < domainNum; ++domain)
                {
                    int localVerDomainNum = verDomainNum[domain];
                    if (localVerDomainNum != 0)
                    {
                        int verNum = 0;
                        int line = 0;
                        for (int line1 = line; line1 < listVtkForSca[domain].Count; ++line1)
                        ...
                        for (int line1 = line; line1 < listVtkForSca[domain].Count; ++line1)
                        {
                            if (listVtkForSca[domain][line1].Contains(scaName_))
                            {
                                ++line1;
                                if (listVtkForSca[domain][line1].Split(' ')[1] == "default")
                                    ++line1;
                        ...
                        scalars[domain] = new float[localVerDomainNum];
                        for (int verIndex = 0; verIndex < localVerDomainNum; ++verIndex)
                        {
                            scalars[domain][verIndex] = value[verDomainIndex[domain][verIndex]];
                            scaMax = Mathf.Max(scalars[domain][verIndex], scaMax);
                            scaMin = Mathf.Min(scalars[domain][verIndex], scaMin);
                ...
                for (int domain = 0; domain < domainNum; ++domain)
                {
                    ...
                    if (localVerDomainNum != 0)
                    {
                        scaColor[domain] = new Color[localVerDomainNum];
                        if (scaMax == scaMin)
                        {
                            for (int verIndex = 0; verIndex < localVerDomainNum; ++verIndex)
                            {
            ...
            for (int vecIndex = 0; vecIndex < vectorsNum; ++vecIndex)
            {
                ...
                vecQua = new Quaternion[domainNum][];
                float vecMax = float.MinValue, vecMin = float.MaxValue;
                string vName = vecName[vecNum[vecIndex]];
                string vecName_ = vName + " ";
                if (vName == "Displacement")
                    verAdd = new Vector3[domainNum][];
                for (int domain = 0; domain < domainNum; ++domain)
                {
                    int localVerDomainNum = verDomainNum[domain];
                    if (localVerDomainNum != 0)
                    {
                        ...
                        for (int line1 = line; line1 < listVtkForSca[domain].Count; ++line1)
                        {
                            if (listVtkForSca[domain][line1].Contains(vecName_))
                            {
                                ++line1;
                                if (listVtkForSca[domain][line1].Split(' ')[1] == "default")
                                    ++line1;
                        ...
                        vectors[domain] = new float[localVerDomainNum];
                        vecQua[domain] = new Quaternion[localVerDomainNum];
                        ...
                        if (vName == "Displacement")
                        {
                            verAdd[domain] = new Vector3[localVerDomainNum];
                            for (int verIndex = 0; verIndex < localVerDomainNum; ++verIndex)
                            {
                                verAdd[domain][verIndex] = new Vector3(1000 * value[3 * verDomainIndex[domain][verIndex]] / (Max - Min),
                                    1000 * value[3 * verDomainIndex[domain][verIndex] + 1] / (Max - Min),
                ...
                for (int domain = 0; domain < domainNum; ++domain)
                {
                    ...
                    if (localVerDomainNum != 0)
                    {
                        vecValue[domain] = new float[localVerDomainNum];
                        if (vecMax == vecMin)
                        {
                            for (int verIndex = 0; verIndex < localVerDomainNum; ++verIndex)
                            {

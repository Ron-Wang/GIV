...
public class CreateContour : MonoBehaviour {
    ...
    int inputScalarNum;
    int outputScalarNum;
    private GameObject panelGiv,panelBg;//可视化界面
    ...
    void onClickOKBg()
    {
        ...
        {
            isBg = true;
            string[] pathOpen = StandaloneFileBrowser.OpenFilePanel("读取.vtk文件", Application.dataPath, "vtk", true);
        ...
        if (isNext)
        {
            panelBg.SetActive(false);//背景界面隐藏
            if(isBg)
            {
